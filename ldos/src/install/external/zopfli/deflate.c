...
static void AddLZ77BlockAutoType(const ZopfliOptions* options, int final,
  ...
  if (expensivefixed) {
    ...
    ZopfliLZ77OptimalFixed(options, &s, lz77->data, instart, inend, &fixedstore);
...
void ZopfliDeflatePart(const ZopfliOptions* options, int btype, int final,
  ...
  if (btype == 0) {
    ...
    ZopfliLZ77OptimalFixed(options, &s, in, instart, inend, &store);
  ...
  for (i = 0; i <= npoints; i++) {
    ...
    ZopfliLZ77Optimal(options, &s, in, start, end, options->numiterations, &store);
