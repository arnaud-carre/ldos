...
void ZopfliFindLongestMatch(ZopfliOptions* options, ZopfliBlockState* s, const ZopfliHash* h,
    ...
    unsigned short* sublen, unsigned short* distance, unsigned short* length)
{
  ...
  int chain_counter = options->maxChainHits;  /* For quitting early. */
...
void ZopfliLZ77Greedy(ZopfliOptions* options, ZopfliBlockState* s, const unsigned char* in,
  ...
  for (i = instart; i < inend; i++) {
    ...
    ZopfliFindLongestMatch(options, s, h, in, i, inend, ZOPFLI_MAX_MATCH, dummysublen,
                           &dist, &leng);
