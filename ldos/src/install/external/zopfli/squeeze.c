...
*/
static double GetBestLengths(ZopfliOptions* options, ZopfliBlockState *s,
  ...
  for (i = instart; i < inend; i++) {
    ...
    ZopfliFindLongestMatch(options, s, h, in, i, inend, ZOPFLI_MAX_MATCH, sublen,
                           &dist, &leng);
...
static void FollowPath(ZopfliOptions* options, ZopfliBlockState* s,
  ...
  for (i = 0; i < pathsize; i++) {
    ...
    if (length >= ZOPFLI_MIN_MATCH) {
      ...
      ZopfliFindLongestMatch(options, s, h, in, pos, inend, length, 0,
                             &dist, &dummy_length);
...
*/
static double LZ77OptimalRun(ZopfliOptions* options, ZopfliBlockState* s,
    const unsigned char* in, size_t instart, size_t inend,
    unsigned short** path, size_t* pathsize,
    unsigned short* length_array, CostModelFun* costmodel,
    void* costcontext, ZopfliLZ77Store* store,
    ZopfliHash* h, float* costs) {
  double cost = GetBestLengths(options, s, in, instart, inend, costmodel,
                costcontext, length_array, h, costs);
  free(*path);
  *path = 0;
  *pathsize = 0;
  TraceBackwards(inend - instart, length_array, path, pathsize);
  FollowPath(options, s, in, instart, inend, *path, *pathsize, store, h);
  assert(cost < ZOPFLI_LARGE_FLOAT);
  return cost;
}

void ZopfliLZ77Optimal(ZopfliOptions* options, ZopfliBlockState *s,
                       const unsigned char* in, size_t instart, size_t inend,
                       int numiterations,
  ...
  /* Initial run. */
  ZopfliLZ77Greedy(options, s, in, instart, inend, &currentstore, h);
  ...
  for (i = 0; i < numiterations; i++) {
    ...
    ZopfliInitLZ77Store(in, &currentstore);
    LZ77OptimalRun(options, s, in, instart, inend, &path, &pathsize,
...
void ZopfliLZ77OptimalFixed(ZopfliOptions* options, ZopfliBlockState *s,
                            const unsigned char* in,
                            size_t instart, size_t inend,
                            ZopfliLZ77Store* store)
{
